...
class GaussianDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def sample_yolo_latent(self, batch_size = 16, clean_img=None,img=None, t=None, input_is_latent=False):
        #print(img.shape)
        
        self.denoise_fn.eval()
        
        if not input_is_latent:
            x_blur_latent = self.yolomodel.module.forward_submodel(img,self.yolomodel.module.before_diffusion_model)
        else:
            x_blur_latent = img
        #print(x_start_latent.shape)
        ...
        while (t):
            step = torch.full((batch_size,), t - 1, dtype=torch.long).cuda()
            x1_bar = self.denoise_fn(x_blur_latent, step)
            x2_bar = self.get_x2_bar_from_xt(x1_bar, x_blur_latent, step)
            ...
            xt_bar = x1_bar
            if t != 0:
                xt_bar = self.q_sample_demix(x_start=xt_bar, x_end=x2_bar, t=step)

            xt_sub1_bar = x1_bar
            if t - 1 != 0:
                step2 = torch.full((batch_size,), t - 2, dtype=torch.long).cuda()
                xt_sub1_bar = self.q_sample_demix(x_start=xt_sub1_bar, x_end=x2_bar, t=step2)

            x = x_blur_latent - xt_bar + xt_sub1_bar
            x_blur_latent = x
            t = t - 1
        
        if clean_img is not None:
            x_start_latent = self.yolomodel.module.forward_submodel(clean_img,self.yolomodel.module.before_diffusion_model)
            if self.loss_type == 'l1':
                loss_xt = (x_start_latent - xt).abs().mean()
                loss_x_direct = (x_start_latent - direct_recons).abs().mean()
                loss_step_x0 = (x_start_latent - x_blur_latent).abs().mean()
                print('sample L1 loss_xt = %.05f, loss_x_direct = %.05f, loss_step_x0 = %.05f'%(loss_xt,loss_x_direct,loss_step_x0))


        self.denoise_fn.train()


        return xt, direct_recons, x_blur_latent #xt, first x0', after state x0'
    ...
    def q_sample_demix(self, x_start, x_end, t):
    ...
    def p_losses_pair(self, x_start, x_blur, t):
        ...
        x_mix_latent = self.q_sample_demix(x_start=x_start_latent, x_end=x_blur_latent, t=t)

        x_recon = self.denoise_fn(x_mix_latent, t)
...
class Dataset_cv2_aug_step(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        #step = randint(0,99)#99 = strong augmentation 
        step = 99 
    ...
    def train(self):
        ...
        while self.step < self.train_num_steps:
            ...
            if self.step != 0 and self.step % self.save_and_sample_every == 0:
                ...
                data_blur = data_blur.cuda()
                step = step.cuda()
                xt, direct_recons, all_images = self.ema_model.module.sample_yolo_latent(batch_size=batches,clean_img = data, img=data_blur)
