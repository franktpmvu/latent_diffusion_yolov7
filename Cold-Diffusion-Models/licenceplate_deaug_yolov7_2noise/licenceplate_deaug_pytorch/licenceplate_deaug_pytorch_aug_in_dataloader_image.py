...
class GaussianDiffusion(nn.Module):
    ...
    def forward_yolo(self, batch_size = 16, img=None, t=None):
        ...
        x_latent,y = self.yolomodel.forward_submodel(x_recon,self.yolomodel.before_diffusion_model,output_y=True)
    ...
    def p_losses_pair(self, x_start, x_blur, t):
        ...
        if self.loss_type == 'l1':
            ...
            x_recon_latent,y = self.yolomodel.forward_submodel(x_recon,self.yolomodel.before_diffusion_model,output_y=True)

            
            x_start_yolo = self.yolomodel.forward_submodel(x_start_latent,self.yolomodel.after_diffusion_model,init_y=y_start)
            
            y[-1]=x_recon_latent
            x_recon_yolo = self.yolomodel.forward_submodel(x_recon_latent,self.yolomodel.after_diffusion_model,init_y=y)
            ...
            #print('loss_image = %.05f, loss_last_yolo = %.05f\n'%(loss_image,loss_last_yolo))
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        if not test_mode:
            wandb.init(
                project="diffusion_image_2loss_512_350k",
                config={
                'ema_decay':ema_decay,
