...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        labels, shapes = get_labels(self.img_files, self.label_files)
        self.shapes = np.array(shapes, dtype=np.float64) # shape = [num_imgs, 2]
        self.labels = list(labels) # shape = [num_imgs, num_objs_per_img, 5]
        ...
        # pbar = tqdm(self.label_files, ncols=150)
        ...
        if cache_images:
            ...
            pbar = tqdm(range(len(self.img_files)), desc='Caching images', ncols=150)
    ...
    def cache_labels(self, path='labels.cache'):
        ...
        x = {}  # dict
        pbar = tqdm(zip(self.img_files, self.label_files), desc='Scanning images', total=len(self.img_files), ncols=100)
...
def get_labels(img_path_list, label_path_list):
    pbar = tqdm(zip(img_path_list, label_path_list), desc='Initializing labels', 
        total=len(img_path_list), ncols=100)
    labels = []
    shapes = []
...
def reduce_img_size(path='../data/sm4/images', img_size=1024):  # from utils.datasets import *; reduce_img_size()
    ...
    for f in tqdm(glob.glob('%s/*.*' % path), ncols=150):
        try:
            img = cv2.imread(f)
            h, w = img.shape[:2]
...
def convert_images2bmp():  # from utils.datasets import *; convert_images2bmp()
    ...
    for path in ['../data/sm4/images', '../data/sm4/background']:
        ...
        for ext in formats:  # ['.bmp', '.jpg', '.jpeg', '.png', '.tif', '.dng']
            for f in tqdm(glob.glob('%s/*%s' % (path, ext)), desc='Converting %s' % ext, ncols=150):
...
def recursive_dataset2bmp(dataset='../data/sm4_bmp'):  # from utils.datasets import *; recursive_dataset2bmp()
    ...
    for a, b, files in os.walk(dataset):
        for file in tqdm(files, desc=a, ncols=150):
            p = a + '/' + file
            s = Path(file).suffix
