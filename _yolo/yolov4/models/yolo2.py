...
class Detect(nn.Module):
    ...
    def forward(self, x):
        for i in range(len(self.m)):
            x[i] = self.m[i](x[i])  # conv # [batch, 39, 64, 64]
        return x 
...
class Model(nn.Module):
    ...
    def info(self):  # print model information
        torch_utils.model_info(self, device='0')


def parse_model(d, ch):  # model_dict(yolov4.yaml), input_channels([3])
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        for j, a in enumerate(args):
            try:
                args[j] = eval(a) if isinstance(a, str) else a  # change strings into int
            except:
                pass
        ...
        if m in [nn.Conv2d, Conv, Bottleneck, SPP, DWConv, MixConv2d, Focus, CrossConv, BottleneckCSP, BottleneckCSP2, SPPCSP, VoVCSP, C3]:
            ...
            args.append([ch[x + 1] for x in f]) # f = [22,26,30], args = [nc, anchors(3,6), [ch[23](number), ch[27], ch[31]]]
