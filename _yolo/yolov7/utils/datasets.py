...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        with h5py.File(path, 'r') as f:
            ...
            label = [np.array(f['label'][k], dtype=np.float32) for k in keys]
            if mode == 'train':
                self.img_keys = [f"img/{k}" for k in keys] + [f"img_blur/{k}" for k in keys]
                self.labels =  label + label
            elif mode == 'test':
                self.img_keys = [f"img/{k}" for k in keys]
                self.labels = label
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            label_key = 'label/' + img_key.split('/')[-1]
