...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        with h5py.File(path, 'r') as f:
            ...
            label = [np.array(f[k], dtype=np.float32) for k in keys if k.endswith('_label')]
            if mode == 'train':
                self.img_keys = [k for k in keys if k.endswith('_bimg')]
                # self.img_keys = [k for k in keys if k.endswith('_img')]
                self.labels = label
            elif mode == 'test':
                self.img_keys = [k for k in keys if k.endswith('_img')]
                self.labels = label
            assert len(self.img_keys)==len(self.labels), "The length of img_keys & labels must be the same."
            self.h5_len = len(self.img_keys)

            shape = []
            for imgk in self.img_keys:
                binary_img = np.array(f[imgk])
                img = io.BytesIO(binary_img)
                img = Image.open(img)
                shape.append(exif_size(img))
            self.shapes = np.array(shape)
        ...
        self.segments = [ [] for _ in range(self.h5_len) ]
        ...
        self.indices = range(self.h5_len)
        ...
        if self.rect:
            ...
            self.img_keys = [self.img_keys[i] for i in irect]
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            label_key = img_key.split('_')[0] + '_label'
            labels = np.array(self.hf[label_key], dtype=np.float32)
            # labels = self.labels[index].copy()
            if labels.size:  # normalized xywh to pixel xyxy format
                labels[:, 1:] = xywhn2xyxy(labels[:, 1:], ratio[0] * w, ratio[1] * h, padw=pad[0], padh=pad[1])
            #print('now is not mosaic')

                
        #print(labels)=[[         10      1799.3      494.31      1811.7      526.46]
        #              [         23      1811.7      491.84      1826.5      526.47]
        #              [         25      1826.6      491.84      1838.9      526.47]
        #              [          6      1841.4      494.31      1853.8      526.46]
        #              [          0      1851.3      491.84      1866.1      526.47]
        #              [          6      1863.7      494.31      1878.5      523.99]
        #              [          1      1878.5      494.31      1888.4      526.46]
        #              [         34      1799.3      489.37      1890.9      528.94]]
        #0~33 words 34=plate
        ...
        # labels_out=[0.00000e+00, 1.00000e+00, 4.73898e-01, 1.25959e-01, 4.74584e-03, 2.08365e-02]
        # self.img_keys[index]=night07_img
        # shapes=((466, 830), ((2.4656652360515023, 2.4674698795180725), (16.0, 17.5)))
        return torch.from_numpy(img), labels_out, self.img_keys[index], shapes
...
def create_dataloader_generator(path, imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
    ...
    dataloader = loader(dataset,
                        ...
                        sampler=sampler,
                        pin_memory=True,
                        collate_fn=LoadImagesAndLabels_generate.collate_fn4 if quad else 
                                    LoadImagesAndLabels_generate.collate_fn)
    return dataloader, dataset
...
class LoadImagesAndLabels_generate(Dataset):  # for training
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        self.aug_licence.imshape=(img_size,img_size)
    ...
    def __len__(self):
        return len(self.image_files)# img_dir = '/data/yolov7/coco/images/train2017' total =118287
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            # generate image with coco
            img, (h0, w0), (h, w), labels, img_key  = self.generate_image(index,train=True)
        ...
        #print(img_key)
        #print(shapes)
        return torch.from_numpy(img), labels_out, img_key, shapes
        # return torch.from_numpy(img), labels_out, self.img_files[index], shapes
    ...
    def generate_image(self, index, train=False):
    ...
    def load_mosaic(self, index):
        ...
        for i, index in enumerate(indices):
            ...
            img, _, (h, w), labels, img_key  = self.generate_image(index,train=True)
            #print(labels)
...
def load_image(self, index):
    ...
    img_key = self.img_keys[index]
    binary_img = np.array(self.hf[img_key])
    img = io.BytesIO(binary_img)
    img = Image.open(img)
    w0, h0 = img.size
    r = self.img_size / max(h0, w0)
    if r != 1:
        img = img.resize((int(w0 * r), int(h0 * r)))
    new_size = img.size
    cv2_img = np.array(img)[:, :, ::-1] #RGB to BGR
    return cv2_img, (h0, w0), new_size[::-1]
