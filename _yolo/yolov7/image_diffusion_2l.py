...
weight = 'runs/train/exp5/best_726.pt'
...
#torch.save(model.state_dict(),'runs/train/exp5/best_726_state_dict.pt')
...
best_726_state_dict='runs/train/exp5/best_726_state_dict.pt'
nc=35
...
def run_aug_img(img_path,times=100):
    ...
    with torch.no_grad():
        while(times>0):
            x_tm1 = aug_licence.batch_data_add_licence_aug(img,(times - 1)*0.01,clamp=True)
            times=times-1
